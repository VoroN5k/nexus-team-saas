    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex flex-col">
      ...
      <nav class="border-b border-slate-700 px-6 py-4 flex items-center gap-4 shrink-0">
        ...
        <div class="flex items-center gap-2">
          ...
          @if (workspace()) {
            <span class="text-xs px-2 py-0.5 rounded-full bg-slate-700 text-slate-400 ml-1">
              {{ myRole() }}
            </span>
          }
        </div>
        <div class="ml-auto flex items-center gap-3">
          <button (click)="activeTab.set('tasks')"
            [class]="activeTab() === 'tasks'
              ? 'text-sm font-medium text-white border-b-2 border-indigo-500 pb-0.5'
              : 'text-sm text-slate-400 hover:text-white transition-colors'">
            Tasks
          </button>
          <button (click)="activeTab.set('members'); loadMembers()"
            ...
            <div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
              <div class="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
                ...
                <form (ngSubmit)="createTask()" #tf="ngForm">
                  ...
                  @if (taskError()) {
                    <p class="mt-2 text-red-400 text-sm">{{ taskError() }}</p>
                  }
                      ...
                      <div class="bg-slate-800 border border-slate-700 hover:border-slate-500 rounded-xl p-4
                        ...
                        <div class="flex items-start justify-between gap-2">
                          ...
                          <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                            ...
                            <button (click)="deleteTask(task)" title="Delete"
                          ...
                          <div class="flex items-center gap-1.5 mt-3">
                            <div class="w-5 h-5 bg-indigo-600 rounded-full flex items-center justify-center">
                              <span class="text-white text-xs font-bold">
                                {{ task.assigned.firstName.charAt(0) }}
                              </span>
                            </div>
        ...
        <div class="flex-1 p-6 max-w-2xl mx-auto w-full">
          ...
          @if (showInvite()) {
            <div class="bg-slate-800 border border-slate-700 rounded-2xl p-5 mb-6">
              <h3 class="font-semibold text-white mb-3">Invite member</h3>
              <form (ngSubmit)="inviteMember()" class="flex gap-3">
                ...
                <button type="submit" [disabled]="inviting()"
                  class="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium rounded-lg transition-colors">
                  {{ inviting() ? '…' : 'Invite' }}
                </button>
              ...
              @if (inviteError()) {
                <p class="text-red-400 text-sm mt-2">{{ inviteError() }}</p>
              }
            </div>
          }
...
export class WorkspaceComponent implements OnInit {
  ...
  tasksByStatus = computed(() => {
    ...
    for (const status of this.statuses) map[status] = [];
    for (const t of this.tasks()) map[t.status]!.push(t);
    return map as Record<TaskStatus, Task[]>;
  });

  myRole = computed(() => this.workspace()?.myRole ?? 'MEMBER');
  ...
  ngOnInit() {
    ...
    this.wsService.getOne(this.workspaceId).subscribe({
      next: (ws) => this.workspace.set(ws as any),
    });
