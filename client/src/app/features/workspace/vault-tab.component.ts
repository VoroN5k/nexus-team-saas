...
export class VaultTabComponent implements OnInit, OnDestroy {
  ...
  private load() {
    ...
    this.wsSvc.getMembers(this.workspaceId).subscribe(ms => this.members.set(ms));
    this.vaultService.listVaults(this.workspaceId).subscribe({
      next: vs => {
        this.vaults.set(vs);
        this.loading.set(false);
        // Load access requests for each vault
        vs.forEach(v => this.loadRequests(v.id));
      },
  ...
  async setupKeys() {
    ...
    try {
      ...
      this.load(); // reload members so publicKey shows up
    } catch (err) {
